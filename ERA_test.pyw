...
if sys.version_info < (2, 6) or sys.version_info > (3, 0):
	...
	try:
		...
		print "Cannot find wxPython\nYou can download wxPython (2.8) from \
	...
	try:
		...
		try:
			...
			print "wxPython 2.8 not found; attempting to use newer version, \
				expect errors"
...
class era(wx.Frame):
	...
	def __init__(self, parent, id):
		...
		self.start_watch = settingsMenu.Append(wx.ID_ANY, 'Start watcher',
			'Start Watcher', kind=wx.ITEM_CHECK)

		settingsMenu.Check(self.debug.GetId(), False)
		settingsMenu.Check(self.start_watch.GetId(), False)

		self.Bind(wx.EVT_MENU, self.Close, fitem)
		self.Bind(wx.EVT_MENU, self.toggle_debug, self.debug)
		self.Bind(wx.EVT_MENU, self.loot_log_watch, self.start_watch)
		...
		era.check_interval = wx.ComboBox(self.panel, -1, '', pos=(65, 10), size=(60, 25), \
			choices=check_interval, style=wx.CB_DROPDOWN)
	...
	def loot_log_watch(self, event):
		if not self.log_watcher:
			self.log_watcher = StartLogWatch(self)
		else:
			self.log_watcher.abort()
			self.log_watcher = None
...
class StartLogWatch(Thread):
	...
	def abort(self):
		print "Stopping the Log Watcher thread\n"
		self._want_abort = 1

	def log_scraper(self, loot_new):
		...
		try:
			...
			print "Could not open %s" % loot_tmp[-1]
		...
		while self._want_abort == 0:
			...
			if new:
				if str("Listener:") in new:
					foo = new.split([3])
					print "Debug: %s\n" % foo

	def run(self):

		log_time = time.strftime('%Y%m%d_', gmtime())[:9]

		loot_path = EVEDir.game_logs

		loot_start = sorted(dict([(f, None) for f in os.listdir(loot_path) if f.startswith("%s" % str(log_time))]))

		print "Watching Gamelogs & Chatlogs for changes\n"

		while self._want_abort == 0:
			time.sleep(1)

			loot_after = sorted(dict([(f, None) for f in os.listdir(loot_path) if f.startswith("%s" % str(log_time))]))

			loot_new = os.path.join(loot_path, loot_start[-1])

			loot_tmp = [f for f in loot_after if not f in loot_start]
			
			if loot_tmp is not None and len(loot_tmp) > 0:
				print "loot_tmp has value: %s" % str(loot_tmp)
			else:
				loot_tmp = loot_new
				continue

			try:
				loot_added = os.path.join(loot_path, loot_tmp[-1])
			except IndexError:
				print "Doesnt look like theres a file yet\n"

			try:
				self.log_scraper(loot_new)
			except Exception as e:
				print e

			print "Added: %s\n" % loot_new

			loot_start = loot_after
